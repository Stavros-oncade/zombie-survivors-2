...
export class EliteEnemy extends Enemy {
  ...
  private eliteState: EliteState = EliteState.CHASE;
  private telegraphTimer?: Phaser.Time.TimerEvent;
  private chargeTimer?: Phaser.Time.TimerEvent;
  private affix: EliteAffix = EliteAffix.SHIELDED;
  ...
  private makeElite(): void {
    ...
    this.setMaxHealth(Math.floor(this.getMaxHealth() * 10));
    this.setHealth(this.getMaxHealth());
    // Try to add glow; otherwise create manual halo
    const hadGlow = this.tryAddGlow(0xff0000, 4, 0, false, 0.1, 16);
    if (!hadGlow) {
    ...
    // Random affix
    const r = Math.random();
    if (r < 0.34) this.affix = EliteAffix.MOLTEN; else if (r < 0.68) this.affix = EliteAffix.SHIELDED; else this.affix = EliteAffix.FROST;
    if (this.affix === EliteAffix.SHIELDED) this.shieldHP = 80;
    if (this.affix === EliteAffix.MOLTEN) {
  ...
  private beginTelegraph(player: Phaser.GameObjects.Sprite) {
    if (this.eliteState !== EliteState.CHASE) return;
    this.eliteState = EliteState.TELEGRAPH;
  ...
  private beginCharge(angle: number) {
    this.clearTelegraph();
    this.eliteState = EliteState.CHARGE;
    ...
    this.chargeTimer = this.scene.time.addEvent({ delay: 800, callback: () => (this.eliteState = EliteState.CHASE) });
  }
  ...
  private leaveMolten() {
    ...
    const player = gameScene ? gameScene.getPlayer() : null;
    if (player) {
  ...
  public override takeDamage(amount: number): void {
    if (this.affix === EliteAffix.SHIELDED && this.shieldHP > 0) {
