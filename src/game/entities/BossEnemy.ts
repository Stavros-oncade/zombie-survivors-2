...
export class BossEnemy extends Enemy {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    (this as any).health = this.maxHP;
    (this as any).maxHealth = this.maxHP;
    this.setScale(1.0);
    this.setTint(0x8844ff);
    if ((this as any).preFX) (this as any).preFX.addGlow(0x8844ff, 6, 0, false, 0.1, 16);
  ...
  private fireProjectileAt(target: Phaser.GameObjects.Sprite) {
    ...
    const s = this.scene.add.sprite(this.x, this.y, this.scene.textures.exists('projectile') ? 'projectile' : undefined);
  ...
  private summonMinions(n: number) {
    const EnemyCls = require('./Enemy').Enemy as typeof Enemy;
    for (let i = 0; i < n; i++) {
      const angle = Math.random() * Math.PI * 2;
      const r = 120 + Math.random()*40;
      const x = this.x + Math.cos(angle)*r;
      const y = this.y + Math.sin(angle)*r;
      const m = new EnemyCls(this.scene, x, y, 0 as any);
      this.scene.add.existing(m);
      (this.scene as any).enemies.add(m);
    }
  }

  private getHealthSafe(): number { return (this as any).getHealth ? (this as any).getHealth() : (this as any).health; }
