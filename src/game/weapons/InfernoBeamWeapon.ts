...
// Inferno Beam: a WIDE beam (much thicker than Prism Beam's thin line) that
// locks onto the nearest threat to pick a firing direction, then damages every
// enemy caught within its width along that segment for low damage — the real
// payoff is that every enemy it touches is unconditionally ignited (unlike
// bomb/airstrike's chance-based ignite, fire IS this weapon's identity).

export class InfernoBeamWeapon implements IWeapon {
  ...
  public fire(scene: Scene, player: Phaser.Physics.Arcade.Sprite, enemies: Enemy[]): void {
    ...
    // Lock the firing direction to the nearest enemy within range.
    ...
    // Pierce all the way through the locked-on target to the beam's full
    // range — the segment used for hit-testing extends past the target
    // rather than stopping at it, so anything behind it (within beamWidth)
    // is hit too, not just the first enemy in the direction fired.
    const angle = Phaser.Math.Angle.Between(player.x, player.y, target.x, target.y);
    const ex = player.x + Math.cos(angle) * this.beamRange;
    const ey = player.y + Math.sin(angle) * this.beamRange;
    const line = new Phaser.Geom.Line(player.x, player.y, ex, ey);
    const burnSystem = (scene as unknown as Game).getBurnSystem?.();

    // Every enemy within beamWidth of the (now full-length) segment takes
    // damage AND is ignited — unconditional, no chance roll: the fire is the
    // point of this weapon.
    for (const e of enemies) {
      if (!e.active) continue;
      const d = this.pointToSegment(e.x, e.y, player.x, player.y, ex, ey);
      if (d <= this.beamWidth) {
        e.takeDamage(dmg);
        if (e.active) burnSystem?.ignite(e, 0);
      }
    }
  ...
  private drawBeam(scene: Scene, line: Phaser.Geom.Line): void {
    ...
    g.setScrollFactor(1);
    g.lineStyle(this.beamWidth, 0xff5500, 0.55);
